			...
			ui.div(
				...
				},
				{
					text: `RPC: ${chalk[cache.ui.defaultColor](
						cache.ui.hideRpc
							? `${cache.config.rpc[Math.floor(Math.random() * cache.config.rpc.length)].slice(
									0,
									5
							  )}...${cache.config.rpc[Math.floor(Math.random() * cache.config.rpc.length)].slice(-5)}`
			...
			ui.div(
				{
					text: `IN:  ${chalk.yellowBright(
						toDecimal(route.amountIn, inputToken.decimals)
			...
			ui.div(
				...
				},
				{
					text: `MIN. OUT: ${chalk.magentaBright(
						toDecimal(route.amountWithFees, outputToken.decimals)
					)}`,
				},
				{
				...
				if (cache?.tradeHistory?.length > 0) {
					...
					tableData.map((entry) =>
						ui.div(
							...
							{
								text: `${entry.expectedamountOut} ${entry.outputToken}`,
								border: true,
							},
							{
